...
class CatalogAggregator:
    def __init__(self, raw_data_dir, mongo_connector, numeric_chunks=50, non_numeric_chunks=10):
    ...
    def aggregate_data(self):
        ...
        self.mongo_connector.insert_aggregated(self.aggregated)
    ...
    def parse_header(self, file_header):
        file_dir = '/'.join(file_header.split(';')[0:-1]).replace(
            'by_name/SAMPLE_ID/DELIVERY_ID/Assembly', 'delivery_dir')
        file_name_and_attributes = file_header.split(';')[-1]
        file_name = '_'.join([file_dir, file_name_and_attributes.split(':')[0]])
        file_attribute = '_'.join(file_name_and_attributes.split(':')[1:])
        return file_name.rstrip('\n'), file_attribute.rstrip('\n')
    ...
    def aggregate_numeric_list(self, original_file_name, original_attribute, values):
    ...
    def aggregate_non_numeric_list(self, original_file_name, original_attribute, values):
        counted = Counter(values)
        if original_file_name not in self.aggregated:
            self.aggregated[original_file_name] = {}

        self.aggregated[original_file_name][original_attribute] = \
            dict(sorted(counted.iteritems(),
                        key=operator.itemgetter(1),
                        reverse=True)[:self.non_numeric_chunks])
